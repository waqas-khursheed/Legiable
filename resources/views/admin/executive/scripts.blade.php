...
*/
    ...
    $(document).on('click', '#executive-list', function(e) {
        e.preventDefault();
        dataRefresh();
    });
    dataRefresh();
    ...
    */
    function dataAdd() {
        $.ajax({
            type: "get",
            url: "{{ url('/admin/executive/create') }}",
            success: function(data) {
                $('#executive').html(data);
            }
        });
    }
    $(document).on('click', '#add-executive', function(e) {
        e.preventDefault();
        dataAdd();
    });
...
*/
    ...
    // Add Quote 
    $(document).on("click", "#btn-add-executive", function(e) {
        e.preventDefault();
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                var form = document.querySelector('#executive-form');
                var data = new FormData(form);

                $.ajax({
                    url: "{{ url('admin/executive/save')}}",
                    type: 'POST',
                    headers: {
...
|--------------------------------------------------------------------------
*/
    $(document).on('click', '#executive-edit', function(e) {
        e.preventDefault();

        var id = $(this).data('id');
        $.ajax({
            url: "{{ url('/admin/executive/edit') }}",
            type: "GET",
            data: {
                id: id,
            },
    ...
    */
    ...
    $(document).on("click", "#btn-update-executive", function(e) {
        e.preventDefault();
        const swalWithBootstrapButtons = Swal.mixin({
            customClass: {
                confirmButton: 'btn btn-success',
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                var form = document.querySelector('#executive-update-form');
                var data = new FormData(form);

                $.ajax({
                    url: "{{ url('/admin/executive/update') }}",
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
...
*/
    function deleteData(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ url('/admin/executive/delete') }}" + '/' + id,
                    type: "get",
                    data: {
                        "_token": "{{ csrf_token() }}",
                    },
