...
*/
    ...
    $(document).on('click', '#my_right-list', function(e) {
        e.preventDefault();
        dataRefresh();
    });
    dataRefresh();
    ...
    */
    function dataAdd() {
        $.ajax({
            type: "get",
            url: "{{ url('/admin/my-right/create') }}",
            success: function(data) {
                $('#my_right').html(data);
            }
        });
    }
    $(document).on('click', '#add-my_right', function(e) {
        e.preventDefault();
        dataAdd();
    });
...
*/
    ...
    // Add Quote 
    $(document).on("click", "#btn-add-my_right", function(e) {
        e.preventDefault();
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                var form = document.querySelector('#my_right-form');
                var data = new FormData(form);

                $.ajax({
                    url: "{{ url('admin/my-right/save')}}",
                    type: 'POST',
                    headers: {
...
|--------------------------------------------------------------------------
*/
    $(document).on('click', '#my_right-edit', function(e) {
        e.preventDefault();

        var id = $(this).data('id');
        $.ajax({
            url: "{{ url('/admin/my-right/edit') }}",
            type: "GET",
            data: {
                id: id,
            },
            success: function(response) {
                $('#my_right').html(response);
            },
            error: function(err) {
    ...
    */
    ...
    $(document).on("click", "#btn-update-my_right", function(e) {
        e.preventDefault();
        const swalWithBootstrapButtons = Swal.mixin({
            customClass: {
                confirmButton: 'btn btn-success',
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                var form = document.querySelector('#my_right-update-form');
                var data = new FormData(form);

                $.ajax({
                    url: "{{ url('/admin/my-right/update') }}",
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
...
*/
    function deleteData(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ url('/admin/my-right/delete') }}" + '/' + id,
                    type: "get",
                    data: {
                        "_token": "{{ csrf_token() }}",
                    },
                    success: function(data) {
                        Swal.fire({
                            position: 'top-end',
                            icon: 'success',
                            title: 'My Right Deleted Successfully',
                            showConfirmButton: false,
                            timer: 1000
                        });
                        dataRefresh();
